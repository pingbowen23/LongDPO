...
if __name__ == "__main__":
    ...
    if config.RM == "dummy":
        ...
        rm_config = RemoteRewardModelConfig(
            ...
            reward_model_addr = config.reward_model_addr
        )
    ...
    def parallel_evaluate_test_dataset(
        ...
        with open(config.input_file, 'r') as file:
            test_ds = [json.loads(line) for line in file]
        
        for i in range(len(test_ds)):
            test_ds[i]["question"] = test_ds[i]["instruction"]
            test_ds[i]["answer"] = test_ds[i]["instruction"]
            test_ds[i]["group_id"] = i + config.group_init_index
            test_ds[i]["save_step_dir"] = config.save_step_dir
            test_ds[i]["save_refine_dir"] = config.save_refine_dir
