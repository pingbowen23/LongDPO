...
def get_mcts_score(outputs):
    ...
    for idx,output in enumerate(outputs):
        try:
            output = extract_info(r'\{(.*?)\}', output.outputs[0].text)
...
@torch.inference_mode()
def reward_function(input_str: str, model, tokenizer, device):
    
    prompt_template = open("/home/pingbowen/workspace/DDPO/DDPO-generate_long/evaluation/judge.txt", "r", encoding="utf-8").read()
    prompt = prompt_template.replace('$INST$', input_str[0]).replace('$RESPONSE$', input_str[1])
    prompt = [{"content": prompt,"role":"user"}]
    input_ids = tokenizer.apply_chat_template(prompt,tokenize=True, add_generation_prompt=True,return_tensors="pt").to(device)
    # prompts = [
    #     prompt_template.replace('$INST$', question).replace('$RESPONSE$', answer)
    #     for question, answer in qa_pairs
    # ]
    outputs = model.generate(input_ids, temperature=1.0, max_new_tokens=16, do_sample=True, top_p=0.95, top_k=50)
    return [0.1]
