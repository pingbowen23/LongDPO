...
import json
from .retrieve import text_split # , get_embedding,encode,mean_pooling
INVALID_ANS = "[invalid]"
...
class CoTEnv(BaseEnv):
    ...
    def __init__(
        ...
        self.memory_pool = ["This is a test","This is a test2"]
    ...
    def add_fact(self,facts):
        self.memory_pool.extend(facts[:2])
    
    def add_generation_prompt(self,prompt):
        return f"<|start_header_id|>user<|end_header_id|>\n\n{prompt}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n"
    
    def extract_info(self, pattern, text):
        import re
    ...
    def get_reward_fn(self, prompts):
        openai_api_key = "EMPTY"
        openai_api_base = f"http://0.0.0.0:9010/v1"
        client = OpenAI(
    ...
    def retrieve_fact(self, action):
        prompt_template = open("", "r", encoding="utf-8").read()
        prompts = [
            self.add_generation_prompt(prompt_template.replace('$RESPONSE$', action))
        ]
        
        # openai_api_key = "EMPTY"
        # openai_api_base = f"http://0.0.0.0:9000/v1"
        # client = OpenAI(
        #     api_key=openai_api_key,
        #     base_url=openai_api_base,
        # )
        ...
        completion = self.get_reward_fn(prompts)
        
        retrieved_facts = []
        
        try:
            output = self.extract_info(r'\{(.*?)\}', completion.choices[0].text)
            outputs = json.loads('{' + output + '}')
            
            for k,v in outputs.items():
                if "statement" in k.lower():
                    retrieved_facts.append(v)
        except:
            pass
    ...
    def check_invalid_fact(self, action):
        prompt_template = open("", "r", encoding="utf-8").read()
        chunks = [chunk["content"] for chunk in text_split(action)]
        
        doc_embeddings = self.get_embedding(chunks)
        query_embeddings = self.get_embedding(self.memory_pool)
        
        d_embs = torch.tensor(doc_embeddings)
        q_embs = torch.tensor(query_embeddings)
        
        d_embs = d_embs / d_embs.norm(dim=1, keepdim=True)
        q_embs = q_embs / q_embs.norm(dim=1, keepdim=True)
        scores = q_embs @ d_embs.T
        # idxs = torch.argsort(scores, dim=-1, descending=True)
        
        threshold = 0.8
        inputs = {"batch":[],"prompts":[]}
        for idx, score in enumerate(scores):
            indices = torch.where(score > threshold)[0]
            query = self.memory_pool[idx]
            for index in indices:
                inputs["batch"].append(idx)
                inputs["prompts"].append(self.add_generation_prompt(prompt_template.replace('$STATEMENT$', query).replace('$CHUNK$', chunks[index.item()])))
        
        if inputs["batch"] == []:
            return False
        
        completion = self.get_reward_fn(inputs["prompts"])
        
        for i in range(len(completion.choices)):
            try:
                output = self.extract_info(r'\{(.*?)\}', completion.choices[i].text)
                outputs = json.loads('{' + output + '}')
                if "Rating" in outputs:
                    if outputs["Rating"] == "invalid":
                        return True
            except:
                pass
        return False
    ...
    def step(self, action, ucb_scores, find_fact, update_legal_action=True):
        if not find_fact:
            self.action_history.append(action)
        else:
            sorted_indices = [index for index, score in sorted(enumerate(ucb_scores), key=lambda x: x[1], reverse=True)]
            
            if self.memory_pool is not []:
                
                for index in sorted_indices:
                    action = self.legal_actions[index]["action"]
                    has_invalid_fact = self.check_invalid_fact(action)
                    if not has_invalid_fact:
                        self.action_history.append(action)
                        facts = self.retrieve_fact(action)
                        self.add_fact(facts)
                        break
                    elif index == len(sorted_indices)-1:
                        self.action_history.append(self.legal_actions[0]["action"])
                # for index in sorted_indices:
                #     facts = self.retrieve_fact(action)
                #     has_invalid_fact = self.check_invalid_fact(facts)
                #     if not has_invalid_fact:
                #         self.action_history.append(action)
                #         self.add_fact(self.memory_pool, facts)
            else:
                self.action_history.append(action)
                facts = self.retrieve_fact(action)
                self.add_fact(facts)
        ...
        actions_copy = copy.deepcopy(self.legal_actions)
        ...
        if not (terminated or truncated) and update_legal_action:
            ...
            if info["winner"] == 1:
                reward = 1.0
            info["api_completion_token"] = 0                    
        return state, reward, terminated, truncated, info , actions_copy
    ...
    def update_legal_actions(self):
        ...
        for i in range(len(texts)):
            ...
            terminated = texts[i].endswith(self.sep)
            processed_act = texts[i]
